...
public class LlmClient {
    ...
    public LlmClient(String apiUrl, String apiKey, String modelName, double temperature, int maxTokens, int timeoutSeconds) {
    ...
    public CompletableFuture<String> askAiAsync(String systemPrompt, List<ConversationMessage> conversationMessages) {
        return askAiAsync(systemPrompt, conversationMessages, this.maxTokens, this.temperature);
    }

    public CompletableFuture<String> summarizeMemoryAsync(ConversationHistory.CompressionRequest request, int maxTokens, double temperature) {
        if (request == null) {
    ...
    private CompletableFuture<String> askAiAsync(String systemPrompt, List<ConversationMessage> conversationMessages, int maxTokens, double temperature) {
        URI uri;
        try {
            uri = URI.create(this.apiUrl);
        } catch (IllegalArgumentException ex) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("LLM API 地址无效: " + this.apiUrl, ex));
        }
        if (!uri.isAbsolute()) {
            return CompletableFuture.failedFuture(new IllegalArgumentException("LLM API 地址必须是完整 URL: " + this.apiUrl));
        }
        ...
        List<ConversationMessage> safeConversationMessages = conversationMessages == null ? List.of() : conversationMessages;
